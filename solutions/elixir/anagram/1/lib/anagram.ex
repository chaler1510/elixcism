defmodule Anagram do
  ...
  defp anagram?(str, low_base, map_base) do
    ...
    if low_str == low_base do
      false
    else
      to_map(low_str) == map_base
    end
  end
  ...
  defp to_map(<<char::utf8, rest::binary>> = str, acc) do
    current = <<char::utf8>>

    if Map.has_key?(acc, current) do
      to_map(rest, acc)
    else
      acc = Map.put(acc, current, String.count(str, current))
      to_map(rest, acc)
    end
  end
end
